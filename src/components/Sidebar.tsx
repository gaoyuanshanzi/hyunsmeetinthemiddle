...
import { Users, Plus, Trash2, Search, RotateCcw, Sparkles, AlertCircle } from 'lucide-react';
...
export const Sidebar: FC<SidebarProps> = ({
  ...
  return (
    <aside className="w-full lg:w-[440px] xl:w-[480px] bg-white border-r border-slate-200 h-full flex flex-col shrink-0 shadow-sm overflow-hidden z-20">
      {/* 탭 및 인원 설정 영역 */}
      <div className="p-4 lg:p-5 border-b border-slate-100 overflow-y-auto max-h-[48vh] lg:max-h-[50vh] space-y-4">
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          <button
            ...
            className="text-[11px] px-2 py-1 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg transition-colors"
          >
            강남·홍대·노원 (3명)
          </button>
          <button
      ...
      {/* 결과 리스트 영역 */}
      <div className="flex-1 overflow-y-auto p-4 lg:p-5 space-y-4 bg-slate-50/50">
        {results.length > 0 ? (
          <>
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-slate-500">카드를 눌러 지도에서 확인</span>
            </div>
          ...
          <div className="h-full flex flex-col items-center justify-center text-center p-6 text-slate-400 space-y-3">
            ...
            <div>
              <p className="text-sm font-semibold text-slate-600">출발역을 입력해주세요</p>
              <p className="text-xs text-slate-400 mt-1 max-w-[240px]">
                2명 이상 출발역을 입력 후 '최적의 중간역 찾기'를 누르면 공평한 최적 모임 장소를 계산합니다.
              </p>
            </div>
          </div>
        )}
