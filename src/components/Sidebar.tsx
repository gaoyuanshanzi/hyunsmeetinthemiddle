...
export const Sidebar: FC<SidebarProps> = ({
  ...
  // 1. 입력 영역 (모바일에서는 mobileTab === 'input' 일 때 표시, 데스크톱 lg에서는 항상 표시)
  const isInputVisibleOnMobile = mobileTab === 'input';
  // 2. 결과 영역 (모바일에서는 mobileTab === 'result' 일 때 표시, 데스크톱 lg에서는 항상 표시)
  const isResultVisibleOnMobile = mobileTab === 'result';
  ...
  return (
    <aside className="w-full lg:w-[440px] xl:w-[480px] bg-white border-r border-slate-200 h-full flex flex-col shrink-0 shadow-sm overflow-hidden z-20">
      {/* 1. 입력 폼 섹션 */}
      <div
        className={`p-4 lg:p-5 space-y-4 border-b border-slate-100 overflow-y-auto ${
          isInputVisibleOnMobile ? 'flex flex-col flex-1 lg:flex-none lg:max-h-[48vh]' : 'hidden lg:block lg:max-h-[48vh]'
        }`}
      >
      ...
      {/* 2. 결과 리스트 & 상세설명 섹션 */}
      <div
        className={`p-4 lg:p-5 space-y-4 bg-slate-50/50 flex-1 overflow-y-auto ${
          isResultVisibleOnMobile ? 'block' : 'hidden lg:block'
        }`}
      >
        {results.length > 0 ? (
          <>
